...
// tool for users to post comments about items, and to build recommendation lists
  ...
  function showPicks($keywords)
	...
	$sql = "SELECT * FROM comments,items WHERE keywords='$keywords' AND items.itemid=comments.itemID ORDER BY rank";
	...
	do
	{
		$itemid = $keyrow["itemid"];
  		$imagesql = "SELECT * FROM images WHERE itemid=$itemid";
   		...
   		$flagurl = 1;
   		if (!$imagerow["url"]) {$url = "http://rhinoplex.org/wrecked/images/noimage.gif"; $flagurl = 0;}
   		else {$url = $imagerow["url"];};
   		echo "<tr class=release1 valign=top>";
   		echo "<td>";
   		echo "<a href=\"viewitem.php?itemselect=$itemid\">";
   		if ($flagurl)
			{ echo "<img src=\"".$url."\" align=left width=20 height=20 border=0></a>";}
   		else
			{if  (parseimage($keyrow["itemid"]))
                	{ echo "</a>";} else {echo "<img src=\"".$url."\" align=left width=20 height=20
border=0></a>";};
                     };

	        echo "<b>".$keyrow["artist"]." - ".$keyrow["title"]." - ".$keyrow["label"]." ".$keyrow["catalog"]." ".$keyrow["format"]."  ";
   echo "<i>$".$keyrow["retail"]."</i></b>";
	        if ($keyrow["quantity"]>0)
   		{
   		echo " (<a
href=\"additem.php?page=item&itemid=".$keyrow["itemid"]."&sort=$sort&lower=$lower&number=$number&desc=$desc&mode=$mode\">ADD
TO CART</a>)";
		   }
   		else
   		{echo " OUT OF STOCK";
...
(0,'$itemID','$username','$message','$keywords','$rank','$year+$month+$day','$expire','$pyear+$pmonth+$pday')";
     ...
     // run SQL against the DB
      ...
      echo "<a href=\"$PHP_SELF?keywords=$keywords&sort=$sort&lower=$lower&number=$number&desc=$desc\">more
comments</a>";
    ...
    // print the list if there is not editing
     ...
     $result = mysql_query("SELECT * FROM comments, items WHERE keywords='$keywords' AND comments.itemID=items.itemid
ORDER BY comments.rank");
...
<a href=\"$PHP_SELF?sort=expiredate&lower=$lower&number=$number&desc=$desc\">ExpireDate</a></td>
       ...
       do
       {
        printf("<tr><td>%s</td> <td><a href=\"admincomment.php?itemID=%s\">%s - %s - %s %s</a></td> <td>%s</td>
<td>%s</td><td><a href=\"admincommentB.php?keywords=$keywords\">%s</a></td>
<td>%s</td><td>%s</td><td>%s</td><td>%s</td>",
        $myrow["commentID"],$myrow["itemID"],$myrow["artist"], $myrow["title"], $myrow["label"], $myrow["format"],
$myrow["username"],$myrow["message"],
$myrow["keywords"],
...
<input type="submit" name="show" value="Show All" class="button1">
     ...
     if (!$keywords) {$keywords = $myrow["keywords"];};
...
".$itemlist["format"];
	...
	if (!$_SESSION["username"]) {$username="cutups";} else {$username=$_SESSION["username"];};
     ...
     Keywords</td>
     <td>
     <input type="Text" name="keywords" value="<? echo $keywords; ?>">
     </td>
     </tr>
