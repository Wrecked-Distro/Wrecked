<?php
   // admindistro_orders_pending.php
   // administer purchases from distros that are not complete
   // written by geoff maddock; updated april 2005
   // to do: integrate all of these pages into one class where visibility is determined by
   //        selecting the sql string rather than seperate pages
   ...
   $distro_orderid = isset($_REQUEST['distro_orderid']) ? $_REQUEST['distro_orderid'] : null;
...
paymentMethod='".$_REQUEST['paymentMethod']."', received_date='".$REQUEST['ryear'].$REQUEST['rmonth'].$_REQUEST['rday']."',
received='$received', description='$description' WHERE distro_orderid='$distro_orderid'";
      echo "Update of ".$_REQUEST['distro_orderid']."\n";
     }
     else
     {
  $sql = "INSERT INTO distro_orders (distro_orderid, distroid, order_cost, shipping_cost, order_date, paid_date, 
paid, paymentMethod, received_date, received, description) VALUES (0,'".$_REQUEST['distroid']."','".$_REQUEST['order_cost']."','".$_REQUEST['shipping_cost']."','".$_REQUEST['oyear'].$_REQUEST['omonth'].$_REQUEST['oday']."','".$_REQUEST['pyear'].$_REQUEST['pmonth'].$_REQUEST['pday']."','".$_REQUEST['paid']."', 
'".$_REQUEST['paymentMethod']."','".$REQUEST['ryear'].$REQUEST['rmonth'].$_REQUEST['rday']."','".$_REQUEST['received']."','".$_REQUEST['description']."')";
 
      echo "Inserting order for ".$_REQUEST['distroid']."\n";
	...
	$sql= "SELECT *, distro_orders.description AS description, DATE_FORMAT(order_date,'%m/%d/%y') AS order_date, 
DATE_FORMAT(paid_date,'%m/%d/%y') AS paid_date, DATE_FORMAT(received_date,'%m/%d/%y') AS received_date FROM 
distro_orders, distributors WHERE distro_orders.distroid=distributors.distroid AND distro_orders.paid=0 ORDER BY 
...
<a href=\"$PHP_SELF?module=$module&sort=paid&lower=$lower&number=$number&desc=$desc\">Paid?</a></td>
             <td><font color=\"ffffff\">
<a href=\"$PHP_SELF?module=$module&sort=paymentMethod&lower=$lower&number=$number&desc=$desc\">Payment Method</a></td>
             <td><font color=\"ffffff\">
...
<a href=\"$PHP_SELF?module=$module&sort=distro_orders.description&lower=$lower&number=$number&desc=$desc\">Description</a></td>
       ...
       do
       {
        printf("<tr><td>%s</td> <td><a href=\"?module=$module&distroid=%s\">%s</a></td> 
...
distro_orderid='$distro_orderid' AND distro_orders.distroid = distributors.distroid";
     ...
     <a href="?module=admindistro.php">Distributor</a></td>
     <td>
...
?>"></td>
    ...
    <select name="rday" size="1" class="form1">
     ...
     <tr><td>
        <input type="hidden" name="module" value="admindistro_orders_pending.php">
        <input type="hidden" name="sort" value="<? echo $sort ?>">
...
<td>$".$itemrow["cost"]."</td><td>".$itemrow["quantity"]."</td>";
        ...
        printf("<td><a
href=\"%s?module=%s&distro_itemid=%s&delete=yes&sort=$sort&lower=$lower&number=$number&desc=$desc\">(DELETE)</a></td><td><a
href=\"%s?module=%s&distro_itemid=%s&distro_orderid=%s&sort=$sort&lower=$lower&number=$number&desc=$desc\">(EDIT)</a></td></tr>",
$PHP_SELF,"admindistro_items.php",$itemrow["distro_itemid"],$PHP_SELF,"admindistro_items.php",$itemrow["distro_itemid"],$itemrow["distro_orderid"]);
