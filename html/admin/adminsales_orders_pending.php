<?php // adminsales_orders_pending.php
   // administer sales orders that still need to be processed
   // written by geoff maddock / updated april 2005

	  ini_set("sendmail_from","sales@wrecked-distro.com");


function calcFees($total,$paymentMethod,$shippingMethod)
{
	// calculates how much overhead cost is involved with the order
	// i want this to include paypal fees, credit card costs,
	// the price of mailers, delivery confirmation etc.
	// i may possibly have additional fields added to each order for
	// some of these options in order to add fees to their total at checkout
	// eg. a checkbox for insurance, delivery confirmation, paypal fee calculation
...
function shipEstimate($sales_orderid)
{
	// stub function for calculating the cost of shipping
	// not 100% accurate, but i'm working on getting it there
	//
	// 1 = media rate
	// 2 = parcel post
	// 3 = priority
	...
	$weight=totalWeight($sales_orderid);
	$shiptype=getShip($sales_orderid);
...
function getShip($sales_orderid)
{
	 // returns the id of the shipping type given a sales order
	dbConnect();

	$sql = "SELECT shipping_method FROM sales_orders WHERE sales_orderid='$sales_orderid'";
	$result = mysql_fetch_array(mysql_query($sql));

	$shiptype=$result["shipping_method"];
	return $shiptype;
...
{
  // emails the customer that their order has shipped
...
//  $body = $username." -\n\n<br><br>We have received your payment, and your order has been shipped.\n<br> Order # ".$sales_orderid." - ".$username."\n<br>";
...
if ($myrow=mysql_fetch_array($result))
{
	$body = $body."\n<br>Order Contents Category - Format - Artist - Title - Label - Condition - Quant - Cost\n";

  do
   {
   $body = $body." ".$myrow["category"]." - ".$myrow["format"]." - ".$myrow["artist"]." - ".$myrow["title"]." - ".$myrow["label"]."
".$myrow["condition"]." - ".$myrow["quantity"]." - $".discountItem($myrow["itemid"],$myrow["discount"])*$myrow["quantity"]."\n";
  ...
  $body=$body."\nSubtotal: $".$order_cost."\nTax: $".$tax_cost."\nShipping: $".$shipping_cost."\nTotal: $".$total."\n\n";
	...
	Thanks for your support! - \n<br>WRECKED diy electronic music &amp; media\nhttp://wrecked-distro.com";
...
{
  // sends an email invoice to the customer given a sales order id
  ...
  $subject = "WRECKED ".$username." Order ".$sales_orderid." Confirmation";
  ...
  $sql = "SELECT *, sales_items.quantity AS quantity, items.quantity AS instock FROM sales_items, items WHERE sales_items.itemid=items.itemid AND sales_items.sales_orderid='$sales_orderid' ORDER BY sales_items.sales_itemid DESC";
  ...
  if ($myrow = mysql_fetch_array($result))
  {
  	...
  	do
	{
	   	...
	   	$body = $body." - ".$myrow["label"]." - ".$myrow["condition"]." - ".$myrow["quantity"]." - $".discountItem($myrow["itemid"],$myrow["discount"])*$myrow["quantity"]."\n";
 	} while ($myrow = mysql_fetch_array($result));
...
{
	case 1: $body = $body."Payment: Cash.  Get cash to me. \n\nWRECKED\n3826 East Street\n<br>Pittsburgh PA 15214\nUSA\n\n";
		break;
	case 3: $body = $body."Payment: Paypal. Follow the link below to pay. \n\nhttps://www.paypal.com/xclick/business=geoff.maddock%40gmail.com&amp;item_name=".$username."+order+".$sales_orderid."&amp;item_number=1&amp;amount=%24".$total."\n\n";
		break;
...
{
 // returns whether the item has been confirmed in stock
...
{
 // returns whether the order has been confirmed in stock
...
{
 // returns whether the order has been marked sent
...
ship_state='".$_REQUEST['ship_state']."', ship_zip='".$_REQUEST['ship_zip']."', ship_country='".$_REQUEST['ship_country']."', cancel='".$_REQUEST['cancel']."' WHERE sales_orderid='".$_REQUEST['sales_orderid']."'";
      ...
      {
		if ((saleConfirm($_REQUEST['sales_orderid']) == 1) && ($_REQUEST['confirm'] == 0))
		{
...
ship_state, ship_zip, ship_country) VALUES (0,'".$_REQUEST['userid']."','".$_REQUEST['oyear']+$_REQUEST['omonth']+$_REQUEST['oday']."','".$_REQUEST['confirm']."','".$_REQUEST['pyear']+$_REQUEST['pmonth']+$_REQUEST['pday']."','".$_REQUEST['paid']."','".$_REQUEST['syear']+$_REQUEST['smonth']+$_REQUEST['sday']."','".$_REQUEST['sent']."','".$_REQUEST['order_cost']."', '".$_REQUEST['tax_submit']."','".$_REQUEST['shipping_cost']."','".$_REQUEST['shipping_method']."','".$_REQUEST['billing_method']."','".$_REQUEST['note']."','".$_REQUEST['ship_name']."','".$_REQUEST['ship_address']."','".$_REQUEST['ship_city']."','".$_REQUEST['ship_state']."','".$_REQUEST['ship_zip']."','".$_REQUEST['ship_country']."'";

      echo "inserting ".$_REQUEST['sales_orderid']."\n";
     ...
     } elseif ($_REQUEST['delete'])
     {
       ...
       // delete a record
        ...
        if ($_REQUEST['confirm'])
        {
          ...
          echo " (<a href=\"$PHP_SELF?module=$module&amp;sales_orderid=$sales_orderid&amp;delete=yes&amp;sort=$sort&amp;lower=$lower&amp;number=$number=desc=$desc&amp;keyword=$keyword&amp;search=$search\">YES</a>
...
FROM sales_orders, users, shipping WHERE sales_orders.cancel = 0 AND sales_orders.userid=users.userid AND
sales_orders.shipping_method=shipping.shippingid AND (sales_orders.paid=0 OR sales_orders.sent=0) ORDER BY $sort ".$sortArray[$desc]." LIMIT $lower,
$number";
	...
	$result = mysql_query($sql);
     ...
     if ($myrow = mysql_fetch_array($result))
     {
       ...
       echo "<tr><td class=\"title1\" colspan=15><b>Current
Sales Orders</b></div></td></tr>\n";
...
<a href=\"$PHP_SELF?module=$module&amp;sort=order_date&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Order&amp;nbsp;Date</a></td>
             <td>
<a href=\"$PHP_SELF?module=$module&amp;sort=confirmed&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Confirm</a></td>
             <td>
...
<a href=\"$PHP_SELF?module=$module&amp;sort=order_cost&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Weight</a></td>
             <td>
<a href=\"$PHP_SELF?module=$module&amp;sort=order_cost&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Est</a></td>
...
href=\"adminusers.php?userid=%s\">%s</a> <a href=\"adminsales_orders.php?search=username&amp;keyword=%s\">#</a></div></td>\n
<td><div class=$divcolor>%s</div></td>\n",$myrow["sales_orderid"], $myrow["userid"],$myrow["username"],$myrow["username"],
$myrow["order_date"]);

        if ($myrow["confirm"]==1) {
        	echo "<td><div class=$divcolor>Yes</div></td>\n";
        } else {
        	echo "<td><div class=$divcolor>No</div></td>\n";
        };

        if ($myrow["paid"]==1) {
        	echo "<td><div class=$divcolor>".$myrow["paid_date"]."</div></td>\n<td><div class=$divcolor>Yes</div></td>\n";}
        else {
        	 echo "<td></td>\n<td><div class=$divcolor>No</div></td>\n";
		};

        if ($myrow["sent"] == 1) {
        	echo "<td><div class=\"$divcolor\">".$myrow["sent_date"]."</div></td>\n<td><div class=\"$divcolor\">Yes</div></td>\n";} else {echo "<td></td>\n<td><div class=$divcolor>No</div></td>\n";
        };

    printf("<td><div class=\"$divcolor\">$%s</div></td>\n <td><div class=\"$divcolor\">$%s</div></td> <td><div class=\"$divcolor\">$%s</div></td>", $myrow["order_cost"], $myrow["tax_cost"], $myrow["shipping_cost"]);

    echo "<td><div class=\"$divcolor\">".$myrow["type"]." ".$myrow["state"]." ".$myrow["zip"]."</div></td>";
	...
	{
	 $total = $myrow["order_cost"] + $myrow["shipping_cost"];
     $total = number_format($total,2,'.','');
 	 printf("<td><div class=\"$divcolor\">$%s</div></td>",$total);
	};

    printf("<td><div class=\"$divcolor\">%s</div></td>",totalWeight($myrow["sales_orderid"]));
    printf("<td><div class=\"$divcolor\">%s</div></td>",shipEstimate($myrow["sales_orderid"]));
    printf("<td></td><td></td>");

    printf("<td><a href=\"?module=$module&amp;sales_orderid=%s&amp;confirm=yes&amp;delete=yes&amp;sort=$sort&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">(DEL)</a></td></form>
...
<td><a href=\"?module=$module&amp;mail=1&amp;sales_orderid=%s&amp;sort=$sort&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">(MAIL)</a></td>
<td><a href=\"?module=printsales_orders.php&amp;sales_order=%s\">(PRINT)</a></td></tr>",
$myrow["sales_orderid"],$myrow["sales_orderid"],$myrow["sales_orderid"],$myrow["sales_orderid"],$myrow["sales_orderid"]);
       } while ($myrow=mysql_fetch_array($result));
       echo "</table>\n";
      }
...
sales_orders.sales_orderid='$sales_orderid' AND sales_orders.userid=users.userid";
     ...
     Fill in all fields to add a new sales order     *'d fields are optional.<p>

     <? echo "<b>Sales Order ".$sales_orderid."</b><p>";?>
...
calcFees($myrow["order_cost"],$myrow["billing_method"],$myrow["shipping_method"]); ?></td>
     </tr>
...
?>
     ...
     <td>
     <select name="cancel" size="1">
     <option value="0" <? if ($cancel=="0") echo "selected"; ?>>No
     <option value="1" <? if ($cancel=="1") echo "selected"; ?>>Yes
     </select>
     </td></tr>
...
<?php
     ...
     $result = mysql_query("SELECT items.label, items.artist, items.title, items.itemid AS itemid, items.catalog,
items.retail, items.cost, items.format, sales_items.sales_itemid, sales_items.sales_orderid, sales_items.itemid,
sales_items.quantity, sales_items.confirm, sales_items.discount AS discount  FROM  items, sales_items WHERE  items.itemid=sales_items.itemid
AND sales_items.sales_orderid='$sales_orderid'");
     ...
     if ($myrow = mysql_fetch_array($result))
     {
       ...
       echo "<tr><td class=\"title1\" colspan='8'><b>Order Contents</b></td></tr>\n";
       echo "<tr class=\"title2\">
             <td><a href=\"$PHP_SELF?sort=order_itemid&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Sales_ItemID</a></td>
             <td><a href=\"$PHP_SELF?sort=distro_orderid&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Sales OrderID</a></td>
             <td><a href=\"$PHP_SELF?sort=itemid&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Item</a></td>
             <td><a href=\"$PHP_SELF?sort=quantity&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Quantity</a></td>
             <td><a href=\"$PHP_SELF?sort=cost&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Cost</a></td>
             <td><a href=\"$PHP_SELF?sort=cost&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">Confirm</a></td>
       		 <td colspan='2'></td>
             </tr>\n";
...
$myrow["label"],$myrow["catalog"],$myrow["quantity"], $price);
        ...
        printf("<td><a href=\"%s?module=adminsales_items.php&amp;sales_itemid=%s&amp;delete=yes&amp;sort=$sort&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">(DELETE)</a></td>
        	<td><a href=\"%s?module=adminsales_items.php&amp;sales_itemid=%s&amp;sales_orderid=%s&amp;sort=$sort&amp;lower=$lower&amp;number=$number&amp;desc=$desc\">(EDIT)</a></td></tr>",
       "index.php", $myrow["sales_itemid"],"index.php",$myrow["sales_itemid"],$myrow["sales_orderid"]);
