...
<?php
   ...
   if ($submit)
   {
    // here if no ID then adding, else we're editing
     ...
     if ($listID)
     {
      $sql = "UPDATE list SET listnamesID='$listnamesID', listitemID='$listitemID', listrank='$listrank', listcomment='$listcomment' WHERE listID='$listID'";
      echo "Update of ".$listID."\n";
     }
     else
     {
       $sql = "INSERT INTO list (listID, listnamesID, listitemID, listrank, listcomment) VALUES (0,'$listnamesID','$listitemID','$listrank', '$listcomment')";
    ...
    // print the list if there is not editing
     ...
     if ($myrow = mysql_fetch_array($result))
     {
       ...
       echo "<tr class=\"title2\">
             <td><a href=\"$PHP_SELF?sort=listID&lower=$lower&number=$number&desc=$desc\">listID</a></td>
             <td><a href=\"$PHP_SELF?sort=listnameID&lower=$lower&number=$number&desc=$desc\">listnameID</a></td>
             <td><a href=\"$PHP_SELF?sort=listitemID&lower=$lower&number=$number&desc=$desc\">listitemID</a></td>
             <td><a href=\"$PHP_SELF?sort=listnamesURL&lower=$lower&number=$number&desc=$desc\">Rank</a></td>
             <td><a href=\"$PHP_SELF?sort=listnamesURL&lower=$lower&number=$number&desc=$desc\">Comment</a></td>
             </tr>\n";
...
$myrow["listrank"], $myrow["listcomment"]);
        ...
        printf("<td><a href=\"%s?listID=%s&delete=yes&sort=$sort&lower=$lower&number=$number&desc=$desc\">(DELETE)</a></td><td><a
